...
class pyAMZON():
            ...
            try:
                ...
                for row in rows:
                    try:
                        ...
                        item_list.append({'invoice': i,'order_date': '*ERROR*','name': '*ERROR*', 'category': '*ERROR*', 'price': '*ERROR*'})
                        pass
            except Exception as e:
                item_list.append({'invoice': i,'order_date': '*ERROR*','name': '*ERROR*', 'category': '*ERROR*', 'price': '*ERROR*'})
                pass
    ...
    def get_browser(self,visible= True):
        ...
        service = Service(executable_path=os.path.join(self.DIR,'chromedriver\win64-132.0.6834.111\chromedriver-win64\chromedriver.exe'))
